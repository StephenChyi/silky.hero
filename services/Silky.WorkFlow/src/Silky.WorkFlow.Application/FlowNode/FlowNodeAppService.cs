...
namespace Silky.WorkFlow.Application.FlowNode
{
    public class FlowNodeAppService : IFlowNodeAppService
    {
        ...
        public FlowNodeAppService(IFlowNodeDomainService flowNodeDomainService, IFlowLineDomainService nodeActionResultDomainService)
        {
        ...
        public Task CreateAsync(CreateFlowNodeInPut flowNode)
        {
            ...
            List<Domain.FlowLine> results = new();
        ...
        //树状拆解为平表
        private void BreakFlowNodeTree(long actId, long id, List<Domain.FlowNode> nodes, List<Domain.FlowLine> results, List<NodeCalculation> calculations, Domain.FlowNode node, NodeActionResultInput[] nextNodes)
        {
            //node.NextNodes = new List<Domain.FlowLine>();//清空实体结构
            //if (nextNodes != null && nextNodes.Any())
            //{
            //    foreach (var action in nextNodes)
            //    {
            //        actId++;
            //        Domain.FlowLine nodeAction = action.Adapt<Domain.FlowLine>();
            ...
            //            nextNode.NextNodes = new List<Domain.FlowLine>();//清空实体结构
            //            nodes.Add(nextNode);
        ...
        private void BuildFlowNodeTree(Domain.FlowNode node, string businessCategoryCode, NodeActionResultInput[] nextNodes)
        {
            //if (nextNodes != null && nextNodes.Any())
            //{
            //    node.NextNodes = new List<Domain.FlowLine>();
            //    foreach (var nextNode in nextNodes)
            //    {
            //        Domain.FlowLine nodeActionResult = nextNode.Adapt<Domain.FlowLine>();
        ...
        //private void BuildFlowNodeTreeDto(long prevFlowNodeId, List<Domain.FlowLine> acts, FlowNodeOutPut prevFlowNodeDto)
        //{
